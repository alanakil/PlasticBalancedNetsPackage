%% Simulation of EE plastic balanced network. Oja's Rule September 2019
%%% Here EE synapses change as in Oja's rule.
    ...
    if(~isempty(Ispike))
        ...
        if(eta~=0)
            ...
            J(Ispike(Ispike<=Ne),1:Ne)=J(Ispike(Ispike<=Ne),1:Ne)+ ... 
                -gamma*repmat(eta*x(1:Ne)',nnz(Ispike<=Ne),1).*(J(Ispike(Ispike<=Ne),1:Ne));
        end
...
mean(nonzeros(J(1:4000,1:4000)))*sqrt(N)
