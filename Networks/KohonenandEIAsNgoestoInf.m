...
NTrials = 4; % Number of data points, i.e., number of different population sizes used.
c = 0;
N_realizations = 2;
...
Tburn = 5000; % burn in period total time (in ms).
% Time discretization
dt=0.1;
timeBP = dt:dt:Tburn;
...
for k = 1:NTrials
    ...
    eta1 = 1/100 / (-200/sqrt(N));
    beta=6/sqrt(N);
    
    % Mean connection strengths between each cell type pair
    Jm=[10 -150; 112.5 -250]/sqrt(N); % Redefine these matrices, because it depends on N.
    Jxm=[180; 135]/sqrt(N);
...
%             clear st;
            ...
            if(~isempty(Ispike))
                ...
                if(eta~=0)
                    ...
                    J(1:Ne,Ispike(Ispike<=Ne))=J(1:Ne,Ispike(Ispike<=Ne))+ ...
                        +eta*beta*repmat((x(1:Ne)),1,nnz(Ispike<=Ne)).*(J(1:Ne,Ispike(Ispike<=Ne))~=0);
                    % E to E after a postsynaptic spike
                    J(Ispike(Ispike<=Ne),1:Ne)=J(Ispike(Ispike<=Ne),1:Ne)+ ...
                        -eta*(J(Ispike(Ispike<=Ne),1:Ne));
                end
                ...
                if(eta1~=0)
                    ...
                    J(1:Ne,Ispike(Ispike>Ne))=J(1:Ne,Ispike(Ispike>Ne))+ ...
                        -repmat(eta1*(x(1:Ne)-alpha),1,nnz(Ispike>Ne)).*(J(1:Ne,Ispike(Ispike>Ne)));
        ...
        Record_Jmean(k,rz) = mean( nonzeros( J(1:Ne,1:Ne) ) );
        Record_Jstd(k,rz) = std( nonzeros( J(1:Ne,1:Ne) ) );
...
save('/scratch/AlanAkil/BurnVariablesCorrKohonen.mat', 'meanFinalCee','meanFinalCei','meanFinalCii',...
    ...
    'Tburn','dt','timeRecord','winsize','c','Theory_Jei','meanXcurrent',...
