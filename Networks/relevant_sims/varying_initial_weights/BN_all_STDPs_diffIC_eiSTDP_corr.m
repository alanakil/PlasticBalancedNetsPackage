...
eta_ei=2/10000/Jmax_ei; % Learning rate of EI
...
Jm_ei_iSTDP_vector = (-250:5:-55)/sqrt(N);% [-250,-200,-150,-100,-50]/sqrt(N);

mean_Jei_recorded = zeros(length(Jm_ei_iSTDP_vector),Ntrec);
dtRate=100; % ms
eRateT = zeros(length(Jm_ei_iSTDP_vector),length(dtRate:dtRate:T));
iRateT = zeros(length(Jm_ei_iSTDP_vector),length(dtRate:dtRate:T));
winsize=250; 
...
IeRecord=zeros(N,length(Jm_ei_iSTDP_vector));
IiRecord=zeros(N,length(Jm_ei_iSTDP_vector));
IxRecord=zeros(N,length(Jm_ei_iSTDP_vector));

reMean = zeros(length(Jm_ei_iSTDP_vector),1);
riMean = zeros(length(Jm_ei_iSTDP_vector),1);

Tburn=T/2;

parfor indx = 1:length(Jm_ei_iSTDP_vector)
    rng(seed);

    Jm_ei = Jm_ei_iSTDP_vector(indx);
    ...
    tic
    J=[Jm(1,1)*binornd(1,P(1,1),Ne,Ne) Jm_ei*binornd(1,P(1,1),Ne,Ni); ...
...
%         JRec_ii_std(1,ii)=1/sqrt(nJrecord0) *sqrt(N)* std(J(sub2ind(size(J),Jrecord_ii(1,:),Jrecord_ii(2,:))));
    ...
    mean_Jei_recorded(indx,:) = JRec_ei*sqrt(N);
...
% Now compute <J><r>.
R_e_theory = zeros(length(Jm_ei_iSTDP_vector),1);
R_i_theory = zeros(length(Jm_ei_iSTDP_vector),1);
for i=1:length(Jm_ei_iSTDP_vector)
    W(1,2) = P(1,2)*mean_Jei_recorded(i,end)*qi;
...
save('/scratch/AlanAkil/diffIC_eiSTDP_Oct8_corr.mat',...
    'mean_Jei_recorded', 'mC','mV','P','R_e_sims','R_i_sims',...
    'mR','Jm_ei_iSTDP_vector','alpha_e','dt','Ne','Ni',...
    'R_e_theory','R_i_theory','reMean','riMean',...
    'N','T','dt','c','dtRate','Jm','Jxm','seed','T1','T2','tauSTDP',...
    'winsize','W','Wx','rx','eRateT','iRateT','Jmax_ei','eta_ei');
