...
%%% This code simulates a single run of a balanced network with iSTDP on
%%% I->E synapses.
...
Jm=[25 -250; 112.5 -250]/sqrt(N);
Jxm=[180; 135]/sqrt(N);
% Jxm=[1.5/0.1/0.2; -5/0.1/0.2]/sqrt(N);
...
tic
% J=[J_NeNe.*normrnd(Jm(1,1),1/sqrt(N),Ne,Ne) J_NeNi.*normrnd(Jm(1,2),1/sqrt(N),Ne,Ni); ...
%     J_NiNe.*normrnd(Jm(2,1),1/sqrt(N),Ni,Ne) J_NiNi.*normrnd(Jm(2,2),1/sqrt(N),Ni,Ni)];
m1=Jm(1,1);
m2=Jm(1,2);
m3=Jm(2,1);
m4=Jm(2,2);
...
m6=Jxm(2);
v = 1/sqrt(N);
% J=[J_NeNe.*lognrnd(log((m1^2)/sqrt(v+m1^2)),sqrt(log(v/(m1^2)+1)),Ne,Ne) -J_NeNi.*lognrnd(log((m2^2)/sqrt(v+m2^2)),sqrt(log(v/(m2^2)+1)),Ne,Ni); ...
%     J_NiNe.*lognrnd(log((m3^2)/sqrt(v+m3^2)),sqrt(log(v/(m3^2)+1)),Ni,Ne) -J_NiNi.*lognrnd(log((m4^2)/sqrt(v+m4^2)),sqrt(log(v/(m4^2)+1)),Ni,Ni)];
J=[Jm(1,1)*binornd(1,P(1,1),Ne,Ne) Jm(1,2)*binornd(1,P(1,1),Ne,Ni); ...
   Jm(2,1)*binornd(1,P(1,1),Ni,Ne) Jm(2,2)*binornd(1,P(1,1),Ni,Ni)];

% Jx=[J_NeNx.*normrnd(Jxm(1),1,Ne,Nx); J_NiNx.*normrnd(Jxm(2),1,Ni,Nx)];
% Jx=[J_NeNx.*lognrnd(log((m5^2)/sqrt(v+m5^2)),sqrt(log(v/(m5^2)+1)),Ne,Nx); J_NiNx.*lognrnd(log((m6^2)/sqrt(v+m6^2)),sqrt(log(v/(m6^2)+1)),Ni,Nx)];
...
% Plasticity params
Jmax = -200/sqrt(N);
eta=75/100000/Jmax; % Learning rate 
tauSTDP=200;
...
record_J = zeros(length(J(1,Ne+1:N)),numel(time));
...
figure;
plot(s(1,s(2,:)>0),s(2,s(2,:)>0),'k.','Markersize',0.1)
xlabel('time (ms)')
ylabel('Neuron index')
...
figure; 
hist(riSim*1000,40)
xlabel('Inhibitory rate (Hz)','fontsize',15)
...
threshold = 80; % cut of to look at the tails of the rate distribution.
t = find(1000*riSim>threshold);

figure;
plot(t,1000*riSim(t),'linewidth',2)
ylabel('Inhibitory rate (Hz)','fontsize',15)
xlabel('Neuron (Inh)','fontsize',15)

figure; histogram(nonzeros(sqrt(N)*J(1:Ne,t(2)+4000)))
ylabel('Count','fontsize',15)
xlabel('j_{ei}','fontsize',15)

%% Look at all connections into a single E neuron.


figure;
hold on
j=1;
for i = 1:length(record_J(:,1))
...
%%
% save('./SavedSimulations/Jei_traces_rates_time.mat', 'riSim', 'record_J');
...
%% Compute mean Jei
mean(nonzeros(J(1:Ne,Ne+1:N)))*sqrt(N)
...
%% Correlations between Jei and ri.
...
Jei_jk = zeros(1,Ni);
for i =1:Ni
    Jei_jk(i) = mean(nonzeros(sqrt(N)*J(1:Ne,Ne+i)));
end

figure; 
hold on

scatter( 1000*riSim ,Jei_jk )
p = polyfit(1000*riSim ,Jei_jk, 1);
linear_fit = polyval(p,1000*riSim);
plot(1000*riSim, linear_fit)

xlabel('Rate of I cells','fontsize',15)
ylabel('j_{ei}','fontsize',15)


%% Compute amount of change in jei vs rtes.

figure; 
plot(1000*riSim, Jei_jk-(Jm(1,2)*sqrt(N)), 'k.')


% save('./Jei_traces_rates.mat', 'riSim', 'JRec','Jrecord','timeRecord');
% 
% 
% save('./SavedSimulations/JeiRi_Corrs_50.mat', 'riSim', 'Jei_jk','linear_fit',...
%     'p','c','N','T');
%%
Jei_dist = sqrt(N)*nonzeros(J(1:Ne,Ne+1:N));
 
% save('./SavedSimulations/Jeidist_200.mat', 'Jei_dist','c','N','T');
