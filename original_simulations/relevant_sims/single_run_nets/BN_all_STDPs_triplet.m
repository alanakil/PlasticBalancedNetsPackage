...
% EE synapses - triplet rule (BCM in spiking)
v_theta = 10/1000 * tauSTDP; % Threshold
eta_ee_triplet=1/1000; % Learning rate of EE
gamma_1=-1;
gamma_2=-1;
    ...
    if(~isempty(Ispike))
        ...
        if(eta_ee_triplet~=0)
            ...
            J(1:Ne,Ispike(Ispike<=Ne))=J(1:Ne,Ispike(Ispike<=Ne))+ ...
                -eta_ee_triplet*repmat(x(1:Ne),1,nnz(Ispike<=Ne))...
                *v_theta*gamma_1...
                .*(J(1:Ne,Ispike(Ispike<=Ne))>0);
            %E to E after a postsynaptic spike
            J(Ispike(Ispike<=Ne),1:Ne)=J(Ispike(Ispike<=Ne),1:Ne)+ ...
                +eta_ee_triplet*(repmat( (x(1:Ne)').^2,nnz(Ispike<=Ne),1)) ...
                .*(gamma_2).*(J(Ispike(Ispike<=Ne),1:Ne)>0);
        end
...
if(eta_ee_soft~=0 || eta_ee_hard~=0 || eta_ee_kohonen~=0 || eta_ee_triplet~=0)
    figure; hold on
