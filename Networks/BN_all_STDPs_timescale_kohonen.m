...
eta_ee_kohonen_vector = [1/10000,1/1000,1/100,1/10,1];

mean_Jee_recorded = zeros(length(eta_ee_kohonen_vector),Ntrec);
dtRate=100; % ms
eRateT = zeros(length(eta_ee_kohonen_vector),length(1:dtRate:T));
iRateT = zeros(length(eta_ee_kohonen_vector),length(1:dtRate:T));
winsize=250; 
T1=T/2; % Burn-in period
T2=T;   % Compute covariances until end of simulation
Ie_current_eta = zeros(length(eta_ee_kohonen_vector),Ntrec);
Ii_current_eta = zeros(length(eta_ee_kohonen_vector),Ntrec);
Ix_current_eta = zeros(length(eta_ee_kohonen_vector),Ntrec);


parfor eta_indx = 1:length(eta_ee_kohonen_vector)
    
    eta_ee_kohonen = eta_ee_kohonen_vector(eta_indx);
    ...
    for i=1:numel(time)
        ...
        if mod(i,T/5) == 0 % print every ten iterations.
            fprintf('At time %d...\n',i*dt/T);
        end
    end
...
save('/scratch/AlanAkil/timescales_analysis_kohonen_ee.mat', 'mean_Jee_recorded', 'mC','mV',...
    'mR','Ie_current_eta','Ii_current_eta','Ix_current_eta','eta_ee_kohonen_vector',...
