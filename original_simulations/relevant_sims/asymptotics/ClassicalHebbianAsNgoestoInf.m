...
Jee_time=cell(NTrials, N_realizations);
...
for k = 1:NTrials
    ...
    Jm=[20 -100; 112.5 -250]/sqrt(N); % Redefine these matrices, because it depends on N.
    Jxm=[180; 135]/sqrt(N);
...
%             clear st;
        ...
        for i=1:numel(timeBP)
            ...
            if(~isempty(Ispike))
                ...
                % If there is Classical Hebbian EE plasticity
                if(eta~=0)
        ...
        Record_Jmean(k,rz) = mean( JRec(length(JRec)/2:end) )*sqrt(N);
        Jee_time{k,rz} = JRec*sqrt(N);
...
r_e_theory = -0.315/(Jee_theory - 45) *1000;
r_i_theory = (0.81-0.0054*Jee_theory)/(45-Jee_theory) *1000;

W(1,1) = Jee_theory*0.8*0.1;
...
%         Theory_Jee = -123.6959;
%     end
%     W(1,2) = Theory_Jee*0.2*P(1,1); % 0.2 is fraction of inh neurons.
% else
%     Theory_Jee=W(1,2)/0.2/P(1,1);
% end
...
save('./ClassicalHebbian_Ninf.mat', 'meanFinalCee','meanFinalCei','meanFinalCii',...
    'Cee_std_error','Cei_std_error','Cii_std_error','Theory_Cee',...
    'Theory_Cei','Theory_Cii','W','Wx','rx','eFR','iFR','Jmax',...
    'eRate_std_error','iRate_std_error','NTrials','Jee_theory',...
    'spikeIndex', 'spikeTimes','meanEcurrent','meanIcurrent',...
    'Tburn','dt','timeRecord','winsize','c','meanXcurrent',...
    'NTrials','Record_Jmean','N_realizations','N_axis',...
    'eRateT', 'iRateT','meanFinalRee','meanFinalRei','meanFinalRii',...
    'Ree_std_error','Rei_std_error','Rii_std_error','Jee_time');
