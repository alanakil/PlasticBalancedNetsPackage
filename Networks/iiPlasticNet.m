%% Simulation of a balanced network with I->I plasticity.
%%%
clear
...
[II,JJ]=find(J(Ne+1:N,Ne+1:N)); % Find non-zero I to E weights
...
threshold = 20; % cut off to look at the tails of the rate distribution.
...
figure; histogram(nonzeros(sqrt(N)*J(Ne+1:N,t(2)+4000)))
ylabel('Count','fontsize',15)
xlabel('j_{ii}','fontsize',15)
...
%% Compute mean Jei
mean(nonzeros(J(Ne+1:N,Ne+1:N)))*sqrt(N)
...
W(2,2) = 0.1*(mean(nonzeros(J(Ne+1:N,Ne+1:N)))*sqrt(N))*0.2;
