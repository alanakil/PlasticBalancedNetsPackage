...
beta = 2/sqrt(N);
eta_ee_kohonen=2/1000; % Learning rate of EE
...
Jm_ee_kohonen_vector = (0.1:0.2:4)/sqrt(N);

mean_Jei_recorded = zeros(length(Jm_ee_kohonen_vector),Ntrec);
dtRate=100; % ms
eRateT = zeros(length(Jm_ee_kohonen_vector),length(dtRate:dtRate:T));
iRateT = zeros(length(Jm_ee_kohonen_vector),length(dtRate:dtRate:T));
winsize=250; 
T1=T/2; % Burn-in period
T2=T;   % Compute covariances until end of simulation
% Ie_current_Jm = zeros(length(Jm_ei_iSTDP_vector),Ntrec);
% Ii_current_Jm = zeros(length(Jm_ei_iSTDP_vector),Ntrec);
% Ix_current_Jm = zeros(length(Jm_ei_iSTDP_vector),Ntrec);

IeRecord=zeros(N,length(Jm_ee_kohonen_vector));
IiRecord=zeros(N,length(Jm_ee_kohonen_vector));
IxRecord=zeros(N,length(Jm_ee_kohonen_vector));

reMean = zeros(length(Jm_ee_kohonen_vector),1);
riMean = zeros(length(Jm_ee_kohonen_vector),1);

Tburn=T/2;

parfor indx = 1:length(Jm_ee_kohonen_vector)
    % We want each run to be the same actual network.
    rng(seed);
    
    Jm_ee = Jm_ee_kohonen_vector(indx);
    ...
    tic
    J=[Jm_ee*binornd(1,P(1,1),Ne,Ne) Jm(1,2)*binornd(1,P(1,1),Ne,Ni); ...
...
%         JRec_ii_std(1,ii)=1/sqrt(nJrecord0) *sqrt(N)* std(J(sub2ind(size(J),Jrecord_ii(1,:),Jrecord_ii(2,:))));
        ...
        % Record average currents to compare <Jr> and <J><r>.
        if i>=T/2/dt
            sum_conv_Spike = sum_conv_Spike + conv_Spike(:,1);
            sum_conv_Spike_X = sum_conv_Spike_X + conv_Spike_X(:,1);
            IeRecord(:,indx) = IeRecord(:,indx) + Ie;
            IiRecord(:,indx) = IiRecord(:,indx) + Ii;
            IxRecord(:,indx) = IxRecord(:,indx) + Ix;
        end
    ...
    mean_Jee_recorded(indx,:) = JRec_ee*sqrt(N);
    
    % Time-dependent mean rates
    eRateT(indx,:)=hist(s(1,s(2,:)<=Ne),dtRate:dtRate:T)/(dtRate*Ne);
    iRateT(indx,:)=hist(s(1,s(2,:)>Ne),dtRate:dtRate:T)/(dtRate*Ni);
    ...
    reMean(indx)=mean(reSim);
    riMean(indx)=mean(riSim);
    disp(sprintf('\nMean E and I rates from sims: %.2fHz %.2fHz',1000*reMean(indx),1000*riMean(indx)))
    ...
    mR{indx}=[mRee mRei; mRei mRii];

    % Save mean E, I, X currents to check for balance.
%     Ie_current_Jm(indx,:) = mean(IeRec);
%     Ii_current_Jm(indx,:) = mean(IiRec);
%     Ix_current_Jm(indx,:) = mean(IxRec);
end
...
% Compute actual <Jr>.
R_e_sims = mean(IeRecord(1:Ne,:)+IiRecord(1:Ne,:)+IxRecord(1:Ne,:),1);
R_i_sims = mean(IeRecord(Ne+1:N,:)+IiRecord(Ne+1:N,:)+IxRecord(Ne+1:N,:),1);

% Now compute <J><r>.
R_e_theory = zeros(length(Jm_ee_kohonen_vector),1);
R_i_theory = zeros(length(Jm_ee_kohonen_vector),1);
for i=1:length(Jm_ee_kohonen_vector)
    W(1,1) = P(1,1)*mean_Jee_recorded(i,end)*qi;
    R_e_theory(i) = (W(1,1) * reMean(i) + W(1,2) * riMean(i) + Wx(1)*rx )*sqrt(N);
    R_i_theory(i) = (W(2,1) * reMean(i) + W(2,2) * riMean(i) + Wx(2)*rx )*sqrt(N);
end

%% Save necessary variables.

save('/scratch/AlanAkil/diffIC_kohonen_Sept18.mat',...
    'mean_Jee_recorded', 'mC','mV','P','R_e_sims','R_i_sims',...
    'mR','Jm_ee_kohonen_vector','dt','Ne','Ni','beta',...
    'R_e_theory','R_i_theory','reMean','riMean',...
    'N','T','dt','c','dtRate','Jm','Jxm','seed','T1','T2','tauSTDP',...
    'winsize','W','Wx','rx','eRateT','iRateT','eta_ee_kohonen');
