...
beta = 2/sqrt(N);
eta_ee_kohonen=1/100; % Learning rate of EE
...
beta_ee_kohonen_vector = 1/sqrt(N) * (1:0.2:4);

mean_Jee_recorded = zeros(length(beta_ee_kohonen_vector),Ntrec);
Jee_recorded = cell(length(beta_ee_kohonen_vector),1);
dtRate=100; % ms
eRateT = zeros(length(beta_ee_kohonen_vector),length(1:dtRate:T));
iRateT = zeros(length(beta_ee_kohonen_vector),length(1:dtRate:T));
winsize=250;
T1=T/2; % Burn-in period
T2=T;   % Compute covariances until end of simulation
Ie_current_beta = zeros(length(beta_ee_kohonen_vector),Ntrec);
Ii_current_beta = zeros(length(beta_ee_kohonen_vector),Ntrec);
Ix_current_beta = zeros(length(beta_ee_kohonen_vector),Ntrec);

parfor beta_indx = 1:length(beta_ee_kohonen_vector)
    
    beta = beta_ee_kohonen_vector(beta_indx);
    ...
    nJrecord1=500; % Number to record
    ...
    % The first row of Jrecord is the postsynaptic indices
    % The second row is the presynaptic indices
    nJrecord0=500; % Number to record
...
%     mean_Jee_recorded(beta_indx,:) = JRec_ee*sqrt(N);
    Jee_recorded{beta_indx} = JRec_ee*sqrt(N);
    
    % Time-dependent mean rates
    eRateT(beta_indx,:)=hist(s(1,s(2,:)<=Ne),1:dtRate:T)/(dtRate*Ne);
    iRateT(beta_indx,:)=hist(s(1,s(2,:)>Ne),1:dtRate:T)/(dtRate*Ni);
    ...
    mC{beta_indx}=[mCee mCei; mCei mCii];
    
    % Mean-field spike count variances.
    mV{beta_indx}=[mVee, mVii];
    ...
    mR{beta_indx}=[mRee mRei; mRei mRii];

    % Save mean E, I, X currents to check for balance.
    Ie_current_beta(beta_indx,:) = mean(IeRec);
    Ii_current_beta(beta_indx,:) = mean(IiRec);
    Ix_current_beta(beta_indx,:) = mean(IxRec);
end
%% Save necessary variables.

save('/scratch/AlanAkil/stability_analysis_kohonen_ee.mat', 'Jee_recorded', 'mC','mV',...
    'mR','Ie_current_beta','Ii_current_beta','Ix_current_beta','beta_ee_kohonen_vector',...
    'N','T','dt','c','dtRate','Jm','Jxm','seed','T1','T2','tauSTDP',...
    'winsize','W','Wx','rx','eRateT','iRateT',...
    'eta_ee_kohonen');

% figure;hold on;
% for i=1:length(beta_ee_kohonen_vector)
%     plot(mean_Jee_recorded(i,:), 'linewidth',2)
% end
% xlabel('Time (s)')
% ylabel('EE weights')
% legend('beta1','beta2','beta3','beta4','beta5','beta6','beta7','beta8')
% 
% figure;hold on;
% for i=1:length(beta_ee_kohonen_vector)
%     plot(1000*eRateT(i,:), 'linewidth',2)
%     plot(1000*iRateT(i,:), 'linewidth',2)
% end
% xlabel('Time (s)')
% ylabel('Rates (Hz)')
% legend('beta1','beta2','beta3','beta4','beta5','beta6','beta7','beta8')
