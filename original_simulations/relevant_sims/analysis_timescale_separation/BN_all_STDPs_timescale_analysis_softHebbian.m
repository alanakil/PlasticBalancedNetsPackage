...
eta_ee_soft=1/10000; % Learning rate of EE
...
beta = 2/sqrt(N);
eta_ee_kohonen=0/1000; % Learning rate of EE
...
eta_ee_soft_vector = [1/10000,1/1000,1/100,1/10,1];

mean_Jee_recorded = zeros(length(eta_ee_soft_vector),Ntrec);
dtRate=100; % ms
eRateT = zeros(length(eta_ee_soft_vector),length(1:dtRate:T));
iRateT = zeros(length(eta_ee_soft_vector),length(1:dtRate:T));
winsize=250; 
T1=T/2; % Burn-in period
T2=T;   % Compute covariances until end of simulation
Ie_current_eta = zeros(length(eta_ee_soft_vector),Ntrec);
Ii_current_eta = zeros(length(eta_ee_soft_vector),Ntrec);
Ix_current_eta = zeros(length(eta_ee_soft_vector),Ntrec);


parfor eta_indx = 1:length(eta_ee_soft_vector)
    
    eta_ee_soft = eta_ee_soft_vector(eta_indx);
    ...
    for i=1:numel(time)
        ...
        if mod(i,T/5) == 0 % print every x iterations.
            fprintf('At time %d...\n',i*dt/T);
        end
    end
    ...
    mean_Jee_recorded(eta_indx,:) = JRec_ee*sqrt(N);
...
save('/scratch/AlanAkil/timescales_analysis_hebbian_ee_soft.mat', 'mean_Jee_recorded', 'mC','mV',...
    'mR','Ie_current_eta','Ii_current_eta','Ix_current_eta','eta_ee_soft_vector',...
    'N','T','dt','c','dtRate','Jm','Jxm','seed','T1','T2','tauSTDP',...
    'winsize','W','Wx','rx','Jmax_ee_soft','eRateT','iRateT');

% figure;hold on;
% for i=1:length(eta_ee_soft_vector)-4
