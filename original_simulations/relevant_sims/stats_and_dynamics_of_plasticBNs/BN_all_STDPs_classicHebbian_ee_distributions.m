...
Jmax_ee_hard = (10^20) /sqrt(N); % Upper constraint
Jmin_ee_hard = - (10^20) /sqrt(N); % Lower constraint
eta_ee_hard=5/1000; % Learning rate of EE
...
%     VRec(:,ii)=VRec(:,ii)+V(Irecord);
    ...
    if mod(i,T/4) == 0 % print every ten iterations.
        fprintf('At time %d...\n',i*dt/T);
    end
end
...
% plot(spikeTimes_E,spikeIndex_E,'.','color','blue','Markersize',0.01)
% plot(spikeTimes_I,spikeIndex_I,'.','color','red','Markersize',0.01)
% xlabel('time (ms)')
% ylabel('Neuron index')
...
% Sample covariances
Cee_distribution = randsample(reshape(C(II<=Ne & JJ<=II),1,[]),1000); 
Cei_distribution = randsample(reshape(C(II<=Ne & JJ>Ne),1,[]),1000); 
Cii_distribution = randsample(reshape(C(II>Ne & JJ>II),1,[]),1000); 
...
% Sample variances
Var_e_distribution = randsample(C(II<=Ne & JJ==II),1000); 
Var_i_distribution = randsample(C(II>Ne & JJ==II),1000); 
...
Ree_distribution = randsample(reshape(R(II<=Ne & JJ<=II),1,[]),1000); 
Rei_distribution = randsample(reshape(R(II<=Ne & JJ>Ne),1,[]),1000); 
Rii_distribution = randsample(reshape(R(II>Ne & JJ>II),1,[]),1000); 
...
% Save mainly: rates, covs, variances, corrs, currents, weights
% distributions.
save('/scratch/AlanAkil/singlerun_classicHebbian_ee_distributions.mat', 'JRec_ee',...
    'mC','mV','mR','IeRec','IiRec','IxRec','Tburn','Tburn_raster',...
    'N','T','dt','c','dtRate','Jm','Jxm','seed','T1','T2','tauSTDP',...
    'winsize','W','Wx','rx','eRateT','iRateT','Jmin_ee_hard','Jmax_ee_hard',...
    'eta_ee_hard','Cee_distribution','Cei_distribution',...
    'Cii_distribution','Var_e_distribution','Var_i_distribution',...
    'Ree_distribution','Rei_distribution','Rii_distribution',...
    'reSim','riSim','spikeTimes_E','spikeIndex_E',...
    'spikeTimes_I','spikeIndex_I');
