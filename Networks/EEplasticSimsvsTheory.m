...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Purpose: To compute correlations and rates in the large N limit and compare
%%% to theory for a plastic E to E (Hebbian STDP) balanced network.
...
NTrials = 10; % Number of data points, i.e., number of different population sizes used.
c = 0.1;
N_realizations = 10;
...
Tburn = 1000000; % burn in period total time (in ms).
% Time discretization
dt=1;
timeBP = dt:dt:Tburn;
...
for k = 1:NTrials
    parfor rz = 1:N_realizations
        ...
        Jm=0.5*[50 -300; 225 -500]/sqrt(N); % Redefine these matrices, because it depends on N.
        Jxm=[180; 135]/sqrt(N);
        Jmax=70/sqrt(N);
        ...
        nrecord0=1; % Number to record from each population
...
%             clear st;
            ...
            if(~isempty(Ispike))
                ...
                if(eta~=0)
                    ...
                    J(1:Ne,Ispike(Ispike<=Ne))=J(1:Ne,Ispike(Ispike<=Ne))+ ...
                        +gamma_minus*repmat(eta*(x(1:Ne)),1,nnz(Ispike<=Ne)).*(J(1:Ne,Ispike(Ispike<=Ne)));
                    % E to E after a postsynaptic spike
                    J(Ispike(Ispike<=Ne),1:Ne)=J(Ispike(Ispike<=Ne),1:Ne)+ ...
                        +gamma_plus*repmat(eta*x(1:Ne)',nnz(Ispike<=Ne),1).*(Jmax-J(Ispike(Ispike<=Ne),1:Ne)).*(J(Ispike(Ispike<=Ne),1:Ne)~=0);
                end
        ...
        Record_Jmean(k,rz) = mean( nonzeros( J(1:0.8*N,1:0.8*N) ) );
        Record_Jstd(k,rz) = std( nonzeros( J(1:0.8*N,1:0.8*N) ) );
        ...
        winsize=250;
        T1=10000; % Burn-in period of 250 ms
        ...
        meanCorrs(k,rz) = mCee(k,rz) + 2* mCei(k,rz) + mCii(k,rz);
        
        if k==NTrials
            spikeTimes{k,rz} = s( 1 ,s(2,:)>0.8*1000*k-500 & s(2,:)<0.8*1000*k+500 ); % Half E and Half I.
            spikeIndex{k,rz} = s( 2 ,s(2,:)>0.8*1000*k-500 & s(2,:)<0.8*1000*k+500);
...
Jee = zeros(NTrials,N_realizations);
...
for i=1:NTrials
    for rz = 1:N_realizations
        ...
        Jee(i,rz) = mean( meanJee{i,rz}(ceil( 9*length(meanJee{i,rz})/10 ):length(meanJee{i,rz})) ); % Only average over the last 1/10 of the time series.
    end
    eFR (i) = mean(meanERate(i,:) ); % Empirical E firing rate
    iFR(i) = mean(meanIRate(i,:) ); % Empirical I firing rate
    finalJee(i) = mean(Jee(i,:) ); % Empirical weights
    meanFinalcorrs(i) = mean( meanCorrs(i,:) ); % Empirical avg corrs
    meanFinalCee(i) = mean( mCee(i,:) ); % Empirical ee corrs
    meanFinalCei(i) = mean( mCei(i,:) ); % Empirical ei corrs
    meanFinalCii(i) = mean( mCii(i,:) ); % Empirical ii corrs
    ...
    J_std_error(i) = std( Jee(i,:) ) / sqrt(N_realizations); % J std error
...
if eta ~=0
    if c==0
        Theory_Jee = gamma_plus*Jmax*sqrt(N)/(gamma_plus-gamma_minus);
    else
        Theory_Jee = gamma_plus*Jmax*sqrt(N)/(gamma_plus-gamma_minus);
    end
    W(1,1) = Theory_Jee*0.8*P(1,1); % 0.8 is fraction of E neurons.
else
    Theory_Jee=W(1,1)/0.2/P(1,1);
end
...
Theory_meanC  = Ctheory(1,1) + 2 * Ctheory(1,2) + Ctheory(2,2); % theoretical avg corrs
...
spikeTimes = spikeTimes{NTrials,1}; % Pick only one realization to save.
spikeIndex = spikeIndex{NTrials,1};
meanEcurrent = meanEcurrent{NTrials,1};
meanIcurrent = meanIcurrent{NTrials,1};
meanXcurrent = meanXcurrent{NTrials,1};



save('/scratch/AlanAkil/EETheorySims.mat', 'meanFinalCee','meanFinalCei','meanFinalCii',...
    ...
    'Tburn','dt','timeRecord','winsize','c','Theory_Jee','meanXcurrent',...
    'NTrials','Record_Jmean','Record_Jstd','N_realizations');
