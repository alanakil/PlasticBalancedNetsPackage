...
beta = 3.8/sqrt(N); 
eta_ee_kohonen=0/100; % Learning rate of EE

% EI synapses
Jmax_ei = -200/sqrt(N); % arbitrary normalizing constant (same order as Jei)
eta_ei= 1/1000 /Jmax_ei; % Learning rate of EI
...
% II synapses
Jmax_ii = -200/sqrt(N);
eta_ii=1/10000/Jmax_ii; % Learning rate of EI
rho0=0.023; % Target rate for i cells.
...
% if(eta_ii~=0)
%     figure; hold on
%     plot(timeRecord/1000,(JRec_ii)*sqrt(N), 'linewidth',1)
%     xlabel('time (s)')
%     ylabel('Mean I to I synaptic weight')
%     
%     plot(timeRecord/1000,mean(JRec_ii)*sqrt(N),'k', 'linewidth',3)
%    
%     figure;
...
% distributions.
save('/scratch/AlanAkil/singlerun_ei_ii_iSTDP_slow_ii.mat', 'JRec_ei',...
    'JRec_ii','mC','mV','mR','IeRec','IiRec','IxRec','Tburn','Tburn_raster',...
    ...
    'spikeTimes_I','spikeIndex_I','Jmax_ei','Jmax_ii');
